...
%Folgende Aussagen:
%\begin{center}
%	Element \textit{geh�rt nicht zur} Menge oder \textit{geh�rt zur} Menge
%	Die Antwort auf die Frage ist \textit{nein} oder \textit{ja}\\
%	Die Bedingung ist \textit{falsch oder wahr}\\
%	Der Wer ist  \textit{0 oder 1} \\
%\end{center}
%sind aus der klassischen Mengenlehren.
%
%Im Fuzzy-Logic sehen die Aussagen anders:
%\begin{center}
%\textbf{$\text{von}$ $\text{"geh�rt nicht zur Menge\textquotedblleft }$ 	$\text{bis}$ $\text{"geh�rt zur Menge\textquotedblleft}$ \\
%$\text{von}$ $\text{nein}$ $\text{bis}$ $\text{ja}$\\
%$\text{von}$ $\text{falsch}$ $\text{bis}$ $\text{wahr}$\\
%$\text{von}$ $0$ $\text{bis}$ $1$ \\
%\end{center}
%umwandeln. }

Zum Verdeutlichen betrachten wir die Menge M der reellen Zahlen, die viel größer sind als 1 \cite{CIKruse:15}. 
